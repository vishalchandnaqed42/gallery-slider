...
const Gallery = ({ data = [], maxWidth = 800, maxHeight = 400, visibleItems = 1, spacing = 10, auto = false, autoInterval = 5000, }) => {
  ...
  useEffect(() => {
    ...
    setWidth(galleryEl.clientWidth);
  }, []);
  ...
  return (
    <Container id="gallery" className="gallery" style={styles.gallery}>
      <Container className="gallery-scroller" ref={galleryRef}>
        ...
        {data.map((item, index) => {
          const { id } = item
          return <GalleryItem key={id} data={item} style={styles.galleryItem} title={`Gallery item - ${id}`} alt={`Gallery item - ${id}`} data-active={(index >= slideStartIndex) && (index < (slideStartIndex + visibleItems))}></GalleryItem>
        })}
      </Container>
